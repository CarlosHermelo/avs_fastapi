...
def get_sistema_prompt_base():
    ...
    try:
        ...
        # Registrar en el log la fuente del prompt
        if source == "base_de_datos":
            logger.info("PROMPT_LOADED: Prompt cargado desde base de datos")
        elif source == "archivo_txt":
            logger.info("PROMPT_LOADED: Prompt cargado desde archivo prompt_fallback.txt")
...
async def handle_complete_analysis(
    ...
    try:
        ...
        def generate(state: MessagesState):
            ...
            system_message_content = get_sistema_prompt_base() + docs_content
            ...
            if not es_valido:
                ...
                system_message_content = reducir_contenido_por_palabras(get_sistema_prompt_base() + docs_content) # Asegurarse que se usa la base + docs para reducir
...
"""
        ...
        try:
            ...
            tokens_prompt_sistema_base = contar_tokens(get_sistema_prompt_base(), model_name)
            ...
            tokens_prompt_sistema_base_error = contar_tokens(get_sistema_prompt_base(), model_name) 
